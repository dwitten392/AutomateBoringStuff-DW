...
#ask if you would only like to select certain cancelations
while True:
    answer = input('Would you only like to select cancellation with 30 or more days since the Begin/Approve date? (Y/N): ').lower()
...
while (answer == "n" or answer == "no"):
    ...
    print('There were ' + str(len(sales_df.loc[sales_df.Contract_Cancel_Days >= 1])) + ' cancellations with with no restrictions.')
    cancel_count = input('How many cancellation samples would you like to select? (0 for no selections): ')
    ...
    try:
        ...
        cancel_sample_index = random.sample(list(sales_df[sales_df.Contract_Cancel_Days >= 1].index), int(cancel_count))
...
print('Your excel file should be saved in the same folder as the script. Enjoy!')
